                    ...
                    <tbody>
                        @foreach ($withdrawals as $key => $withdrawal)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('messages.create', ['customerId' => $withdrawal->customer_id]) }}">
                                        {{ $withdrawal->customer->name ?? 'N/A' }}
                                    </a>
                                </td>
                                <td>{{ number_format($withdrawal->customer->Balance ?? 0, 0, ',', '.') }} VND</td>
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-primary btn-sm" data-toggle="modal"
                                        data-target="#updateStatusModal{{ $withdrawal->id }}">
                                        Cập nhật trạng thái
                                    </button>
                                </td>
                            </tr>
                            ...
                            <!-- Modal -->
                            <div class="modal fade" id="updateStatusModal{{ $withdrawal->id }}" tabindex="-1" role="dialog"
                                aria-labelledby="updateStatusModalLabel{{ $withdrawal->id }}" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="updateStatusModalLabel{{ $withdrawal->id }}">Cập nhật
                                                trạng thái</h5>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">&times;</span>
                                            </button>
                                        </div>
                                        <form action="{{ route('admin.withdrawals.updateStatus', $withdrawal->id) }}"
                                            method="POST">
                                            @csrf
